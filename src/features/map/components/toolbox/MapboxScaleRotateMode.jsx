import * as Constants from './mapboxGlLocalFiles/constants';
import doubleClickZoom from './mapboxGlLocalFiles/double_click_zoom';
import createSupplementaryPoints from './mapboxGlLocalFiles/create_supplementary_points';
import * as CommonSelectors from './mapboxGlLocalFiles/common_selectors';
import moveFeatures from './mapboxGlLocalFiles/move_features';
...
SRMode.onSetup = function (opts) {
  ...
  if (
    ...
    feature.type === Constants.geojsonTypes.MULTI_POINT
      ...
      // toast.warning("No se pueden escalar Puntos!")
      return;
  }
  //   if (
  ...
  const addImageIfNotExists = (map, id, src) => {
    // Verificar si la imagen ya está agregada
    if (!map.hasImage(id)) {
      // Si no está agregada, cargarla y luego agregarla
...
SRMode._createRotationPoint = function (
  ...
  var heading = turf.bearing(rotCenter, cR0);
  var distance0 = turf.distance(rotCenter, cR0);
  var distance1 = radiusScale * distance0; // TODO depends on map scale
  var cR1 = turf.destination(rotCenter, distance1, heading, {}).geometry.coordinates;
...
SRMode.computeRotationCenter = function (state, polygon) {
  var center0 = turf.center(polygon);
  return center0;
};
...
SRMode.computeAxes = function (state, polygon) {
  ...
  for (var i1 = 0; i1 < n; i1++) {
    ...
    const c0 = corners[i0];
    const c1 = corners[i1];
    const rotPoint = turf.midpoint(turf.point(c0), turf.point(c1));
    ...
    if (SRCenter.Opposite === state.rotatePivot) {
      ...
      const c2 = corners[i2];
      const c3 = corners[i3];
      rotCenter = turf.midpoint(turf.point(c2), turf.point(c3));
    }

    rotateCenters[i1] = rotCenter.geometry.coordinates;
    headings[i1] = turf.bearing(rotCenter, rotPoint);
  }
...
SRMode.dragRotatePoint = function (state, e, delta) {
  ...
  var heading1 = turf.bearing(center, m1);
  ...
  var rotatedFeature = turf.transformRotate(state.rotation.feature0, rotateAngle, {
    pivot: center,
    mutate: false,
  });
...
SRMode.dragScalePoint = function (state, e, delta) {
  ...
  var dist = turf.distance(center, m1, { units: 'meters' });
  ...
  var scaledFeature = turf.transformScale(state.scaling.feature0, scale, {
    origin: cCenter,
    mutate: false,
  });
